...
import restApi.http.HttpClientFactory;
...
public class MyTest extends BaseTest {
    ...
    @Test
    public void testGet(){
        Request request =  Request.Builder.getInstance()
                .withUri("https://reqres.in/api/users?page=1")
                .withHttpMethod(HttpMethod.GET)
                .build();

        //Response response = httpClient.execute(request);
        Response response = HttpClientFactory.executeRequest(request.getHttpMethod()).execute(request);
